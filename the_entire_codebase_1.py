...
if len(sys.argv) != 2:
    print("Incorrect usage.\nCorrect usage: python <script.py> <subject_directory_name>")
    sys.exit("Incorrect usage.")

SUBJECT_SAVE_DIR = sys.argv[1]
try:
    os.mkdir(SUBJECT_SAVE_DIR)
except:
    print("Error while creating subject directory.")
    sys.exit("Error while creating subject directory.")
...
def display_rating_screen(emotion):
...
positive_images = os.listdir(os.path.join(IMG_MASTER_DIR, 'PositiveLow'))
negative_images = os.listdir(os.path.join(IMG_MASTER_DIR, 'NegativeHigh'))
...
ratings_path = os.path.join(SUBJECT_SAVE_DIR, "ratings.csv")
...
for i in range(N_TRIALS):
    ...
    for emotion_dir in IMG_SUBDIRS:
        ...
        match emotion_dir:
            ...
            case _:
                print("Error in match statement while selecting emotion category.")
                sys.exit("Emotion category match statement error.")
...
eye_tracking_data_save_path = os.path.join(SUBJECT_SAVE_DIR, "eye_tracking_data.csv")
df_gaze.to_csv(eye_tracking_data_save_path, index=False)
